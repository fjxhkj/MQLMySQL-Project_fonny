...
int MySqlConnect(string pHost, string pUser, string pPassword, string pDatabase, int pPort, string pSocket, int pClientFlag)
{
   ...
   if (SQLTrace)
      Print("Connecting to Host=", pHost, ", User=", pUser, ", Database=", pDatabase, " DBID#", connection);
   ...
   if (connection == -1)
   {
      ...
      if (SQLTrace)
         Print("Connection error #", MySqlErrorNumber, " ", MySqlErrorDescription);
   }
   else
   {
      MySqlLastConnect = TimeCurrent();
      if (SQLTrace)
         Print("Connected! DBID#", connection);
   }
   ...
   if (!MySqlExecute(connection, "SET NAMES UTF8"))
   {
      ...
      MySqlErrorDescription = cGetMySqlErrorDescription(-1);
      if (SQLTrace)
         Print("'SET NAMES UTF8' error #", MySqlErrorNumber, " ", MySqlErrorDescription);
   }
...
void MySqlDisconnect(int &pConnection)
{
   ...
   if (pConnection != -1)
   {
      ...
      if (SQLTrace)
         Print("DBID#", pConnection, " disconnected");
      pConnection = -1;
   }
}
...
bool MySqlExecute(int pConnection, string pQuery)
{
   ...
   if (SQLTrace)
   {
      Print("DBID#", pConnection, ", CMD:", pQuery);
   }
   if (pConnection == -1)
   {
      // no connection
      ...
      if (SQLTrace)
         Print("CMD>", MySqlErrorNumber, ": ", MySqlErrorDescription);
      return (false);
   }
...
int MySqlCursorOpen(int pConnection, string pQuery)
{
   ...
   if (SQLTrace)
   {
      Print("DBID#", pConnection, ", QRY:", pQuery);
   }
   ClearErrors();
   ...
   if (result == -1)
   {
      ...
      MySqlErrorDescription = cGetMySqlErrorDescription(pConnection);
      if (SQLTrace)
         Print("QRY>", MySqlErrorNumber, ": ", MySqlErrorDescription);
   }
   return (result);
}
...
int MySqlCursorRows(int pCursorID)
{
   ...
   MySqlErrorDescription = cGetCursorErrorDescription(pCursorID);
   if (SQLTrace)
      Print("Cursor #", pCursorID, ", rows: ", result);
   return (result);
}
